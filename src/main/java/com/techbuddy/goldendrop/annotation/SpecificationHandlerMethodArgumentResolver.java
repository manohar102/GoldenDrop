...
public final class SpecificationHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public boolean supportsParameter(MethodParameter parameter) {
        return BaseSpecificationsBuilder.class.equals(
                parameter.getParameterType().getSuperclass());
    }
    ...
    public Object resolveArgument(
        ...
        BaseSpecificationsBuilder builder =
                (BaseSpecificationsBuilder) parameter.getParameterType().newInstance();
        String operationSetExper = Joiner.on("|").join(SearchOperation.SIMPLE_OPERATION_SET);
        Pattern pattern = Pattern.compile("(\\w+?)(" + operationSetExper
                + ")(\\p{Punct}?)([a-zA-Z_0-9#\\.\\-\\&\\\\^\\(\\)\\%\\\\$\\s\\p{L}]+)(\\p{Punct}?),");
