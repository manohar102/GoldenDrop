...
public class AwsS3Service {
    ...
    public AwsS3Service(AwsS3Config awsS3Config) {
        this.amazonS3 = AmazonS3ClientBuilder.standard()
                .withEndpointConfiguration(
                        new AwsClientBuilder.EndpointConfiguration(awsS3Config.getEndPoint(), awsS3Config.getRegion()))
    ...
    public Optional<URL> putObject(String filePath, InputStream inputStream) {
        ...
        try {
            ...
            PutObjectRequest putObjectRequest =
                    new PutObjectRequest(awsS3Config.getBucketName(), filePath, byteArrayInputStream, metadata);
            this.amazonS3.putObject(putObjectRequest);
            presignedUrl = this.generatePreSignedUrlIfExists(
                    awsS3Config.getBucketName(),
                    filePath,
                    DateTime.now().plus(AwsS3Service.MAX_EXECUTION_TIME).toDate(),
                    HttpMethod.GET);
        } catch (IOException e) {
    ...
    public Optional<URL> generatePreSignedUrlIfExists(
            String bucketName, String filePath, Date expiration, HttpMethod method) {
